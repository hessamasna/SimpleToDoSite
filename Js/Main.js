...
function plus() {
    ...
    var isDone, taskText, time, isBookmark, dateSplit;
    if (newTaskDateAndTime === "") {
        ...
        var date = new Date(taskDate[0].split("-")[0], taskDate[0].split("-")[1], taskDate[0].split("-")[2], taskDate[1].split(":")[0], taskDate[1].split(":")[1]);
        time = findWeekDay(date.getDay()) + ", " + findmonth(date.getMonth()) + " " + taskDate[0].split("-")[2] + " " + taskDate[1].split(":")[0] + ":" + taskDate[1].split(":")[1];
        dateSplit = {
